...
const handler = MiddleHandler(async (req, res) => {
  ...
  if (isExist) return ErrorHandler(res, 400, "Already Exists");
  if (isExistName) return ErrorHandler(res, 400, "Already Exists");

  const doesConnect = await User.findOne({ name: crushName });
  if (doesConnect) {
    Mail(
      email,
      doesConnect.email,
